...
async function get_new_message(message: string) {
    ...
    for (const [index, entry] of profiles_and_messages.entries()) {
        const embedding = await open_ai_embedding_call(entry.message);  // entry.message
    ...
    const vectorDb = await make_faiss_index(textEmbeddings);

    // Get key profile elements.
    // let extraction_prompt = llmSystem.get_formatted_prompt("profile_extraction_prompt", dict(raw_profile=new_profile))
    let extraction_prompt = llmSystem.get_formatted_prompt("profile_extraction_prompt", { raw_profile: new_profile });

    extraction_prompt = extraction_prompt.to_openai();
    let extracted_profile = open_ai_chat_call(extraction_prompt);

    // Get similar profiles
    let profile_query = llmSystem.get_formatted_query("get_similar_profile_id", { new_profile: extracted_profile })
    let similar_profiles_and_messages: any = search_faiss_index(vectorDb, profiles_and_messages, profile_query)

    // Get similar profile messages
    // let sim_messages = [entry["message"] for entry in similar_profiles_and_messages];
    let sim_messages = similar_profiles_and_messages.map((entry: any) => entry.message);

    // Get description of how to write those messages
    let message_description_prompt = llmSystem.get_formatted_prompt("messages_to_description_prompt", { messages: sim_messages }).to_openai()
    ...
    let new_message_prompt = llmSystem.get_formatted_prompt("write_new_message_prompt", { 
        profile1: profile1, 
        profile2: profile2, 
        message1: message1, 
        message2: message2, 
...
(async () => {
    try {
        await get_new_message('Bobo Rules');
