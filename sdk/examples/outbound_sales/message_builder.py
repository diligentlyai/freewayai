...
def get_new_message(name: str) -> str:
    ...
    extraction_prompt = llm_system.get_formatted_prompt("profile_extraction_prompt", raw_profile=new_profile)
    extraction_prompt = extraction_prompt.to_openai()
    extracted_profile = open_ai_chat_call(extraction_prompt)


    # Get simillar profiles
    profile_query = llm_system.get_formatted_query("get_similar_profile_id", new_profile=extracted_profile)
    ...
    message_description_prompt = llm_system.get_formatted_prompt("messages_to_description_prompt", messages=sim_messages).to_openai()
